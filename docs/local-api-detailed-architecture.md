...
- Les décisions de lecture directe et de vérification ACL sont écrites dans
  `.local/audit/access-decisions.jsonl`, hors Git, limité à 1 Mo avec une
  sauvegarde et des permissions privées. Les routes de chat, RAG et recherche
...
8. Une lecture documentaire ou décision ACL est journalisée avant son retour ;
   si le journal est indisponible, l'opération est refusée.
...
- La journalisation active couvre la lecture directe et la vérification ACL,
  mais pas encore le chat, le RAG ou la recherche.
